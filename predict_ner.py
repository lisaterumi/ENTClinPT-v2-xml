    ...
    for i, sentenca in enumerate(sentencas_tokenized):
        ...
        for j, token in enumerate(sentenca):
	        ...
	        for k, MODEL_DIR in enumerate(MODELS_DIR):
	                ...
	                print('classe:',predictedModels[k][i][j].strip())
	                print('fechaTag:',fechaTag)
	                if predictedModels[k][i][j].strip().lower() != 'o':
                            temLabel=1
                            if fechaTag==1:
	                            result = result +' ' +token.strip()
	                            # se é ponto final da sentenca
	                            if j+1 == len(sentenca):
	                                result = result +'</'+nomeTag+'>'
                            else:
	                            #ver se é um termo composto por varias palavras, se começa com B é o primeiro.. se for S, é unico
	                            fechaTag=1
	                            nomeTag=predictedModels[k][i][j].strip()
	                            nomeTag=nomeTag.replace("B-","").replace("I-","").replace("S-","").replace("E-","")
	                            result = result + ' '+'<'+nomeTag+'>'+token.strip()
        	        if temLabel==0:
        	            #print('fechaTag:',fechaTag)
        	            if fechaTag==1:
        	                 fechaTag=0
        	                 result = result +'</'+nomeTag+'>'
        	                 #print('cai aki, fechando taggg')
        	            result = result +' ' +token.strip()
